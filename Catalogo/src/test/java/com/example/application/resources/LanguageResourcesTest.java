...
class LanguageResourcesTest {
	...
	void testGetAllString() throws Exception {
		List<Language> lista = new ArrayList<>(Arrays.asList(
				new Language(1, "Alemán"),
				new Language(2, "Chino"), 
				new Language(3, "Castellano")));
		
		when(srv.getAll()).thenReturn(lista);
		mockMvc.perform(get("/api/lenguages/v1").accept(MediaType.APPLICATION_JSON))
		.andExpectAll(
				status().isOk(),
	...
	void testGetOne() throws Exception {
		...
		mockMvc.perform(get("/api/lenguages/v1/{id}", id))
	...
	void testGetOne404() throws Exception {
		...
		mockMvc.perform(get("/api/lenguages/v1/{id}", id)).andExpect(status().isNotFound())
